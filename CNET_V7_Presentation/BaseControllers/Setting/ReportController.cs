...
namespace CNET_V7_Presentation.BaseControllers.SettingSchema
{
    ...
    public class ReportController : ControllerBase
    {
        ...
        {
            var obj = await _serviceManager.reportService.FindById(id);
        ...
        {
            var accounts = await _serviceManager.reportService.FindAll(trackChanges: false);
            if(accounts.Success)
        ...
        public async Task<IActionResult> CreateReport([FromBody] ReportDTO report)
        {
            ...
            var createdObj = await _serviceManager.reportService.Create(report);
        ...
        {
            if (report is null) return BadRequest("report is null");
            var updatedObject = await _serviceManager.reportService.Update(report);
        ...
        {
            var deletedObject = await _serviceManager.reportService.Delete(id);
